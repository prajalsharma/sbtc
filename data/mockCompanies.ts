export const mockCompanies = [
  {
    _id: "1",
    name: "Alchemy",
    description: "Powerful APIs, SDKs, and tools to build and scale your web3 app with ease.",
    size: "100-1000 employees",
    industry: ["Infra & Tools"],
    location: ["San Francisco, California", "New York City Area"],
    image: "https://pbs.twimg.com/profile_images/1947425842738384896/eDE44AUv_400x400.jpg",
  },
  {
    _id: "2",
    name: "OpenZeppelin",
    description:
      "A provider of security and development tools for blockchain projects, known for its widely-used smart contract libraries and auditing services that ensure the safety of decentralized applications.",
    size: "100-1000 employees",
    industry: ["Infra & Tools"],
    location: ["New York City Area", "Princeton, New Jersey"],
    image: "https://pbs.twimg.com/profile_images/1831782443839344640/d5BLSyQT_400x400.png",
  },
  {
    _id: "3",
    name: "Ready",
    description:
      "Ready makes crypto easy. Buy, swap, spend, earn & invest. The onchain alternative to your bank.",
    size: "10-100 employees",
    industry: ["Wallet"],
    location: ["Paris, France"],
    image: "https://pbs.twimg.com/profile_images/1024585501901303808/m92jEcPI_400x400.jpg",
  },
  {
    _id: "4",
    name: "Starknet",
    description:
      "Starknet is a Validity Rollup that provides unlimited scale while retaining Ethereum's security and decentralization.",
    size: "100-1000 employees",
    industry: ["Blockchain"],
    location: ["Tel Aviv, Israel"],
    image: "https://pbs.twimg.com/profile_images/1948191490804514816/0IFLjyb7_400x400.png",
  },

  {
    _id: "5",
    name: "Immunefi",
    description:
      "Onchain security network with bug bounties and audits protecting billions in web3/DeFi projects.",
    size: "11-50 employees",
    industry: ["Security"],
    location: ["Singapore"],
    image: "https://pbs.twimg.com/profile_images/1904479852733841408/N1hUWlg6_400x400.jpg",
  },
  {
    _id: "6",
    name: "Ritual",
    description:
      "The Ritual chain is the first blockchain custom-built to support AI-native operations, designed from the ground up to enable a new class of applications at the intersection of crypto and AI.",
    size: "11-50 employees",
    industry: ["AI"],
    location: ["Noida, India"],
    image: "https://pbs.twimg.com/profile_images/1912582510631858176/-Xbw2AcT_400x400.jpg",
  },
  {
    _id: "7",
    name: "Braavos",
    description:
      "Crypto wallet for StarkNet offering secure, user-friendly asset management and dApp connections.",
    size: "11-50 employees",
    industry: ["Wallet"],
    location: ["Tel Aviv, Israel"],
    image: "https://pbs.twimg.com/profile_images/1846554119777013760/FydsgAUR_400x400.jpg",
  },
  {
    _id: "8",
    name: "Zellic",
    description:
      "Web3 cybersecurity firm providing audits and cryptography reviews to secure DeFi and blockchain protocols.",
    size: "51-200 employees",
    industry: ["Security"],
    location: ["Remote (distributed)"],
    image: "https://pbs.twimg.com/profile_images/1715242418457747456/FkBeZCZ5_400x400.jpg",
  },
  {
    _id: "9",
    name: "Spiko",
    description:
      "API-driven treasury yield platform for European businesses optimizing cash management and returns.",
    size: "11-50 employees",
    industry: ["RWA"],
    location: ["Paris, France"],
    image: "https://pbs.twimg.com/profile_images/1919771760875937792/IIKL6y3R_400x400.jpg",
  },
  {
    _id: "10",
    name: "Software Mansion",
    description:
      "Polish software dev company building scalable apps and contributing to open-source projects.",
    size: "201-500 employees",
    industry: ["Infra & Tools"],
    location: ["Kraków, Poland"],
    image: "https://pbs.twimg.com/profile_images/1243176655172009986/Jgdl2m15_400x400.jpg",
  },
  {
    _id: "11",
    name: "Hypernative",
    description:
      "Web3 security platform focusing on real-time threat detection and attack prevention.",
    size: "51-200 employees",
    industry: ["Security"],
    location: ["Herzliya, Israel"],
    image: "https://pbs.twimg.com/profile_images/1576848429711560705/zWLmrreo_400x400.png",
  },
  {
    _id: "12",
    name: "QuickNode",
    description:
      "Cloud blockchain infrastructure service providing fast APIs and tooling for Web3 developers and enterprises.",
    size: "100-200 employees",
    industry: ["Infra & Tools"],
    location: ["Miami, USA (remote)"],
    image: "https://pbs.twimg.com/profile_images/1875136807781662720/MJP7n4UN_400x400.jpg",
  },
  {
    _id: "13",
    name: "GetBlock",
    description:
      "Blockchain node and API service offering reliable access to multiple chains for decentralized app development.",
    size: "51-200 employees",
    industry: ["Infra & Tools"],
    location: ["Global (HQ Cyprus)"],
    image: "https://pbs.twimg.com/profile_images/1877312953776582656/g78tZlgn_400x400.jpg",
  },
];
